...
final class WeekTeamInteractor: WeekTeamInteractorProtocol {
    ...
    func updateTeam(basedOn index: Int) {
        if shouldCallApi {    
            self.coordinator.showLoading { [weak self] in
                guard let self else { return }
                self.worker.getCurrentAvailableSortedTeams(completion: { whiteTeam, blackTeam in
                    guard let whiteTeam,
                          let blackTeam else {
                    ...
                    self.coordinator.removeLoading {
                        self.presenter.updateFirebaseModelToViewModel(
                            whiteTeam: whiteTeam,
                            blackTeam: blackTeam)
                    }
                })
